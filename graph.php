...
<div id="chartContainer"></div>
  ...
  <script>
    // Sample data
    var data = [
      { date: '2015-06-28', value: 45 },
      { date: '2018-06-28', value: 45 },
      { date: '2020-06-28', value: 45 },
      { date: '2021-06-28', value: 45 },
      { date: '2022-06-28', value: 45 },
      { date: '2023-06-28', value: 45 },
      { date: '2023-07-01', value: 50 },
      { date: '2023-07-05', value: 25 },
      { date: '2023-07-07', value: 30 },
      { date: '2023-07-10', value: 20 },
      { date: '2023-07-11', value: 20 },
      { date: '2023-07-12', value: 20 },
  ...
  switch (filterType) {
    ...
    case 'monthly':
      ...
      filteredData.forEach(function(entry) {
        var entryMonth = new Date(entry.date).toLocaleString('default', { month: 'long' });
    ...
    case 'weekly':
      ...
      while (dateCursor <= weekEnd) {
        var formattedDate = dateCursor.toLocaleDateString('default', { month: 'short', day: 'numeric' });
      ...
      data.forEach(function(entry) {
        ...
        if (entryDate >= weekStart && entryDate <= weekEnd) {
          var formattedDate = entryDate.toLocaleDateString('default', { month: 'short', day: 'numeric' });
...
function updateChart(filterType, filterYear = null) {
  ...
  var chartData = Object.keys(groupedData).map(function (key) {
    return { x: key, y: groupedData[key] };
  });

  chart.updateSeries([{ data: chartData }]);
  chart.updateOptions({ xaxis: { categories: Object.keys(groupedData) } });
}
